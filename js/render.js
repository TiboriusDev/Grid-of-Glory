...
function renderSidebar(){
  const facA=FACTIONS[pickedFactions.a], facB=FACTIONS[pickedFactions.b];
  ...
  [{k:'move',l:'1· Bewegen'},{k:'attack',l:'2· Angreifen'}].forEach(({k,l})=>{
    ...
    b.addEventListener('click',()=>{if(phase==='over')return;phase=k;sel=null;hlM=[];hlA=[];combat=null;renderGame();});
    pb.appendChild(b);
  });
  ...
  if(phase==='over'){
    ...
    tl.innerHTML=`<span style="color:${fac.color}">${fac.icon} ${fac.name} am Zug</span>`;
  }
  ...
  if(phase==='over'){
    mkBtn(ac,'🔄 Neu starten',()=>{showFaction();});
    renderLog();return;
  }
  ...
  if(combat){
    ...
    if(step==='roll_atk'){
      ...
      ar.forEach(v=>{const d=document.createElement('div');d.className='die '+(v>=hitThresh?'ok':'no');d.textContent=v;row.appendChild(d);});
      dp.innerHTML=`<div class="dice-title">🎯 Angriff — <b>${hits} Treffer</b> (grün = ${hitThresh}+)</div>`;
      dp.appendChild(row);
      const hint=document.createElement('div');hint.style.cssText='font-size:11px;color:var(--text-secondary);margin:4px 0 6px;line-height:1.4;';
  ...
  if(sel&&phase==='attack'&&!sel.attacked){
    ...
    if(ts.length){
      const lbl=document.createElement('div');lbl.style.cssText='font-size:10px;color:var(--text-muted);margin:4px 0 2px;';
      ...
      const lbl=document.createElement('div');lbl.style.cssText='font-size:11px;color:var(--text-muted);margin-top:4px;';
...
function renderGame(){renderBoard();renderSidebar();renderLegend();}

// ══════════════════════════════════════════════════════
// FACTION SCREEN
// ══════════════════════════════════════════════════════
function showFaction(){
  pickedFactions={a:null,b:null};factionPickStep='a';
...
function renderFactionScreen(){
  ...
  lbl.innerHTML=factionPickStep==='a'
    ...
    :`<span style="color:#185FA5">🔵 Spieler 1: <b>${FACTIONS[pickedFactions.a].icon} ${FACTIONS[pickedFactions.a].name}</b></span><br><span style="color:#993C1D">🔴 Spieler 2 wählt sein Volk:</span>`;
...
function renderMapScreen(){
  ...
  [{k:'p0',l:'Offenes Feld'},{k:'p1',l:'Ruinen'},{k:'p2',l:'Fluss'},{k:'ed',l:'✏️ Editor'}].forEach(({k,l})=>{
    ...
    b.textContent=l;b.addEventListener('click',()=>{activeMapTab=k;renderMapScreen();});
    tabs.appendChild(b);
  });
  content.innerHTML='';
...
function buildMini(m){
  ...
  const um={};m.starts.a.forEach((pos,i)=>{um[`${pos[0]},${pos[1]}`]={team:'a'};});
  m.starts.b.forEach((pos,i)=>{um[`${pos[0]},${pos[1]}`]={team:'b'};});
...
document.getElementById('btn-map-start').addEventListener('click',()=>{
  ...
  if(activeMapTab.startsWith('p')){
    ...
    const etm={};edTerrain.forEach(({c,r,t})=>{etm[`${c},${r}`]=t;});
    const free=[];
    for(let r=0;r<edRows;r++)for(let c=0;c<edCols;c++)if(!(etm[`${c},${r}`]>0))free.push([c,r]);
